## ADR-014. AdSense 승인 준비를 위한 정책·소개 페이지와 GitHub 프로필
...
- **About**: GitHub `jon890` 프로필 런타임 fetch와 ISR 1시간을 사용한다.
  실패 시 텍스트로 대체하고 [구조화 로그](../code-architecture.md#로깅과-오류)를 남긴다.
  `next.config.ts`에 `avatars.githubusercontent.com`을 추가한다.
- **Contact**: 이메일 `jon89071@gmail.com`과 GitHub Issues `jon890/fos-study/issues`
...
**도메인 신청 단위**: 이 결정에서는 AdSense 신청 단위를 루트 도메인(`fosworld.co.kr`)으로 정했다.
서브도메인(`blog.*`) 단독 입력 대신 루트 승인 후 사이트 메뉴에서 서브도메인을 추가하고 별도 심사 없이 광고를 노출하는 절차를 전제했다.
