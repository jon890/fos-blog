...
**운영 규칙**: ADR 1개는 파일 1개(`docs/adr/NNN-slug.md`)와 이 INDEX 의 링크 한 줄로 이뤄진다.
새 ADR 은 `docs/adr/NNN-slug.md` 를 만들고 아래 적절한 분류에 링크를 추가한다.
...
- [ADR-001](./001-infinite-scroll-page.md) — 무한 스크롤 페이지 (SSR 첫 페이지와 클라이언트 fetch 하이브리드)
...
- [ADR-022](./022-about-page.md) — About 페이지 co-located CSS 와 2단계 avatar
...
- [ADR-025](./025-series-system.md) — 시리즈 메타데이터, 순서 정책, 목록 경로
- [ADR-030](./030-multi-category.md) — 경로 주 카테고리와 frontmatter 추가 카테고리를 함께 사용
...
- [ADR-036](./036-thin-category-noindex.md) — 내용이 얇은 카테고리 페이지를 색인에서 제외 (plan060)
...
- [ADR-017](./017-design-system.md) — 절제된 개발 도구형 디자인, npm 글꼴, CSS 동작 효과와 자체 토큰
- [ADR-029](./029-design-md.md) — `DESIGN.md`를 도입하고 실제 토큰은 `globals.css`에서 관리
...
- [ADR-034](./034-description-extraction.md) — 글 요약은 산문 블록만 모으고 링크가 있는 첫머리 인용문은 제외 (plan058)
