...
- **결정**: AI agent 가 디자인을 일관되게 생성하도록 [Google Stitch DESIGN.md 컨벤션](https://github.com/voltagent/awesome-design-md) 9섹션 형식의 자립형 `docs/design.md` 를 도입한다. 단 토큰 값의 단일 소스는 `src/app/globals.css` 의 `@theme` 과 `:root` 블록으로 유지하고, design.md 는 사람·외부 agent 용 스냅샷으로 둔다. 값 불일치 시 `globals.css` 가 우선한다.
- **맥락**: AGENTS.md 가 코딩 agent 지침을 표준화하듯 DESIGN.md 가 디자인 agent 지침을 표준화하는 흐름. fos-blog 의 디자인 결정([ADR-017](./017-design-system.md))과 토큰은 `globals.css` 와 여러 ADR 에 흩어져 있어, 외부 agent 가 한 번에 읽을 진입점이 없었다.
- **대안 기각**:
