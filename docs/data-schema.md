...
**구현 범위:** 관리자 인증 테이블 네 개와 study 테이블 13개, 추천·게시·가져오기 서비스와 관리자 공부 화면을 제공한다.
Route Handler는 이 스키마의 원자 저장과 이력 조회 기능을 서비스 계층으로 호출한다.
관리자 공부 화면은 HTTP 계약을 통해 자료와 이력을 조회하거나 개인 상태를 수정한다.
