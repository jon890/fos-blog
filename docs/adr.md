...
- [ADR-030](#adr-030) — 다중 카테고리 (경로 primary + frontmatter 추가, 카테고리 페이지 cross-post 노출, plan051)
...
- [ADR-029](#adr-029) — DESIGN.md 자립형 도입 + globals.css 를 토큰 source of truth 로 유지
...
- [ADR-026](#adr-026) — Markdown 렌더 sanitize 도입 (rehype-sanitize + defaultSchema 확장, plan029)
- [ADR-027](#adr-027) — KaTeX 수식 렌더링 도입 (remark-math + rehype-katex output:"html", plan044)

### 컴포넌트 분리

- [ADR-028](#adr-028) — 시리즈 인덱스 카드는 PostCard variant 가 아닌 별도 SeriesCard 컴포넌트 (plan047)
...
- **맥락**: AGENTS.md 가 코딩 agent 지침을 표준화하듯 DESIGN.md 가 디자인 agent 지침을 표준화하는 흐름. fos-blog 의 디자인 결정([ADR-017](#adr-017))과 토큰은 `globals.css`·`design-inspiration.md`·여러 ADR 에 흩어져 있어, 외부 agent 가 한 번에 읽을 진입점이 없었다.
- **대안 기각**:
