...
## ADR-013. 메인 도메인 전환 — `blog.fosworld.co.kr` 단일화
...
- 배포 절차 (nginx 설정 변경, `.env` 반영, 컨테이너 재기동 등) 는 **저장소 외부에서 관리** — 홈서버 인프라 세부 설정을 공개 저장소에 기록하지 않음
...
## ADR-014. AdSense 승인 요건 — 정책/소개 페이지 + GitHub 프로필 연동
