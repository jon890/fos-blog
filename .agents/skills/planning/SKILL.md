...
이 skill의 원본은 `.agents/skills/planning` 이다. `.claude/skills/planning` 은 같은 디렉터리를 가리키는 symlink 여야 하며, 두 위치에 내용을 복사해 분기하지 않는다.

- Claude Code에서 선택지를 물을 때는 `AskUserQuestion` 을 사용한다.
- Claude Code가 아닌 환경에서는 사용 가능한 질문 도구를 쓰고, 없으면 정확히 하나의 간결한 plain-text 질문으로 묻는다.
- 특정 에이전트 종류를 skill 본문에서 일반 지정하지 않는다. 구체 도구가 필요한 경우에만 해당 도구 사용법을 명시한다.
...
- **선택지는 현재 에이전트의 질문 도구로 제시**: 옵션 중 하나를 고르게 할 때는 위 "공용 원본 및 도구 어댑터"를 따른다. 1~4개 질문, 옵션 2~4개, 추천안은 첫 번째 + label 끝 `(추천)`. 글로 늘어놓는 long-form 옵션 비교는 사용자가 답변 작성에 시간 들이게 됨 — 클릭 한 번으로 끝나도록 인터랙티브화. 단, 결정이 이미 명확하거나 자유 답변이 필요한 경우 (예: 카피 문구) 는 일반 질문 사용
...
#### D. 거울 구조 원칙 (단일 소스 + docs-verifier 흡수)

같은 체크리스트를 두 곳에 유지하면 시간이 지나며 한쪽만 갱신되는 사고 발생.

규칙:

1. **단일 소스**: planning 8단계의 "변경 유형별 docs 영향 표" + 위 "문서 책임 표" 가 docs 갱신의 유일한 정의
...
4. **표 수정 시 거울 동기 검토** — `.claude/agents/fos-blog-docs-verifier.md` 의 도메인 지식 섹션과 함께 갱신
...
- `docs/adr/NNN-slug.md` 신규 생성 + `docs/adr/README.md` 에 링크 추가 (자명성 통과한 것만)
...
1. **docs 반영 완료 확인** — `docs/adr/` / `docs/flow.md` / `docs/data-schema.md` / `docs/code-architecture.md` / `docs/pages/{page}.md` / `CLAUDE.md` 중 해당하는 문서에 이번 결정이 모두 기록됐는지 점검
...
# 열린 PR 이 점유한 plan/ADR 번호 확인 — 로컬 main 에 없어 놓치기 쉬움
gh pr list --state open --json number,headRefName,title --jq '.[] | "\(.headRefName) \(.title)"'
```

다음 가용 번호를 사용. 단순 task 생성 시 항상 위 명령으로 사전 확인.
머지 안 된 열린 PR 의 브랜치가 점유한 번호는 로컬 main 에 없어 놓치기 쉽다.
plan052 가 실측으로 plan051(열린 PR #176)·ADR-030 점유를 이 점검으로 회피했다.
