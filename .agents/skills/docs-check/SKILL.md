...
이 skill의 원본은 `.agents/skills/docs-check` 이다. `.claude/skills/docs-check` 는 같은 디렉터리를 가리키는 symlink 여야 하며, 두 위치에 내용을 복사해 분기하지 않는다.

- Claude Code에서는 `.claude/agents/fos-blog-docs-verifier.md` custom agent를 우선 사용한다.
- Codex에서는 `.codex/agents/fos-blog-docs-verifier.toml` custom agent가 사용 가능하면 우선 사용한다.
- custom agent를 사용할 수 없는 환경에서만 agent 본문을 reference로 읽고, main session이 아래 grep과 5축 점검을 직접 실행한다.
- skill 경로 점검은 `.agents/skills/*/SKILL.md`를 원본으로 본다. `.claude/skills/*/SKILL.md`는 symlink 호환 경로이므로 깨진 symlink 여부만 추가 확인한다.
...
docs-check 의 5축 검증은 가능한 환경에서는 custom agent `fos-blog-docs-verifier` 에 위임한다.
Claude Code에서는 `.claude/agents/fos-blog-docs-verifier.md`, Codex에서는 `.codex/agents/fos-blog-docs-verifier.toml` 을 사용한다.
agent 본문이 검증 항목·자동 grep 명령·도메인 지식의 단일 소스 — main session 이 직접 grep 을 베끼는 순간 정의 두 곳 동기화 부담 발생.
...
2. custom agent 사용 불가 환경 — agent 본문을 reference로 읽고, main session으로 아래 1~5 단계 legacy 경로를 직접 실행
...
grep -rnE "매트릭스|matrix" docs/ .agents/skills/ CLAUDE.md 2>/dev/null && echo "VIOLATION: 매트릭스 용어 — 표/분류 표 등으로 정정"
```
