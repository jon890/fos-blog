...
docs-check 의 5축 검증은 **반드시** custom agent `fos-blog-docs-verifier` (`.claude/agents/fos-blog-docs-verifier.md`) 에 위임. agent 본문이 검증 항목·자동 grep 명령·도메인 지식의 단일 진실원 — main session 이 직접 grep 을 베끼는 순간 정의 두 곳 동기화 부담 발생.
...
#    bash 3.2 호환 위해 tr 로 소문자화 (${var,,} 는 bash 4+ 전용 — macOS 기본 bash 미지원)
for n in $BODY; do
...
# 5) 본문 30 줄 초과 ADR 자동 검출 — "기능 명세서 변질" 의심 (B 과대화)
#    plan033 시점 일부 ADR 이 비대화한 사례 회피. awk 의 끝 매칭이 다음 --- 까지 흘러가지 않도록
#    구분선 누락 먼저 검증.
