...
plan 디렉터리 명명은 `tasks/{plan}/` 정확 일치 외에도 슬러그 suffix 형태 (`tasks/plan044-katex-math/` 등) 가 일반적이라 **3단 fallback** 으로 매칭한다. 1차 실패 시 2차 / 3차 순서로 확장.
...
# 2차 — glob 매칭 (tasks/{plan}-*/) — 슬러그 suffix 의 일반 케이스
[ -z "$PLAN_DIR" ] && PLAN_DIR=$(ls -d tasks/{plan}-*/ 2>/dev/null | head -1 | sed 's|tasks/||; s|/||')

# 3차 — fuzzy grep (대소문자/오타 보호)
[ -z "$PLAN_DIR" ] && PLAN_DIR=$(ls tasks/ 2>/dev/null | grep -i "^{plan}" | head -1)

# 모두 실패 → TASK_MISSING
[ -z "$PLAN_DIR" ] && echo "TASK_MISSING" && exit 0
echo "PLAN_DIR=$PLAN_DIR"
jq -r .status "tasks/$PLAN_DIR/index.json" 2>/dev/null
```
...
**다중 매칭 발견 시** (예: `plan007` → `tasks/plan007-rate-limit/` + `tasks/plan007-2-rate-limit-policy-tightening/`) 2차 단계에서 `ls -d tasks/{plan}-*/ | wc -l` ≥ 2 이면 단독 결정 금지 — `AskUserQuestion` 으로 어느 디렉터리를 대상으로 진행할지 사용자에게 확인 (실사례: plan044 첫 시도가 정확 일치만 시도해 `tasks/plan044-katex-math/` 첫 매칭 실패, 두 번째 ls 시도로 발견).

이후 본 SKILL 절차의 모든 `tasks/{plan}/...` 경로는 `tasks/$PLAN_DIR/...` 로 치환해 진행 (예: `tasks/plan044-katex-math/index.json`).
...
| **executor** | `fos-blog-executor` | sonnet | phase 순차 실행, 코드 수정 (커밋 제외), `bypassPermissions` |
...
이 harness 는 **Claude Code native teams** 모델이다.
`TeamCreate` 도구는 없다. `Agent` 호출에 `team_name` 인자를 직접 넣으면 `agent-spawn-guard.sh` 통과 + 팀이 자동 형성된다.

critic / executor / code-reviewer / docs-verifier는 반드시 **`team_name` 인자로 등록된 팀원**으로 스폰. 일회성 `Agent` 호출(team_name 없이) 금지.
...
각 executor 는 phase 보고에 아래 4종을 함께 적는다.
없으면 "없음" 으로 명시한다 — 침묵으로 갈음하면 사용자가 후속 필요 여부를 판단할 수 없다.
...
- **범위 외 발견** — plan 범위 밖이지만 후속이 필요한 발견. 위 "executor scope 확장 보고 의무" 가 자체 수정을 차단하는 짝이라면, 이 항목은 발견 내용을 사용자에게 전달하는 누적 채널이다.

team-lead 는 종료 시 phase 별 특이사항을 누적해 사용자에게 명시 보고한다.
...
이 harness 는 **Claude Code native teams** 모델이다.
`TeamCreate` 도구는 없다.
`Agent` 호출에 `team_name` 인자를 직접 넣으면 팀이 자동 형성된다.
...
7. **OPEN PR 의 미머지 변경 동시 검토 (필수 — plan036 시간차 사고 관측)**: docs 재작성 plan (예: data-schema 전면 재작성, flow 라우트 표 개편) 의 docs-verifier 는 origin/main 만 보면 안 됨. 동일 파일을 변경하는 OPEN PR 들의 변경분도 함께 검토해야 합병 후 빠진 항목 (예: plan036 이 plan033 의 series 컬럼을 누락) 이 발생 안 함. 검증:
   ```bash
   # docs/data-schema.md / docs/adr/ / docs/flow.md 등 핵심 docs 를 변경하는 OPEN PR 목록
...
8. **ADR 파일 ↔ INDEX 정합성 (필수 — plan033 / plan044 drift 관측)**: 새 ADR 추가(`docs/adr/NNN-slug.md` 신규 생성) 또는 `docs/adr/` 변경 plan 에서 docs-verifier 는 다음을 확인.
   - `docs/adr/[0-9]*.md` 파일 목록과 `docs/adr/README.md` INDEX 링크가 일치하는지 (파일 있으나 INDEX 누락, 또는 INDEX 에 있으나 파일 없음)

   검증:
   ```bash
   # ADR 파일 목록 vs README INDEX 등재 누락
   ...
   # 출력 있으면 파일 ↔ index 불일치 — UPDATE_NEEDED
   ```
