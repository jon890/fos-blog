...
## 1-10. 코드 / 주석 / describe 에 plan 번호 / task 번호 참조 (PR #156)

**증상**: 코드 주석 / CSS 주석 / vitest `describe` 블록 이름에 `(plan044)` / `// ... — plan029` 같은 task 번호 흔적.
```ts
// Bad
// KaTeX (plan044) — output:"html" 모드의 fallback 영역 aria-hidden 보존
describe("KaTeX math rendering (plan044)", () => { ... });
```
**왜**: CLAUDE.md "Don't reference the current task, fix, or callers" 규칙 위반. plan 참조는 시간이 지나면 의미를 잃고 (a) AI 가 "이 코드가 왜 여기 있나" 추론을 plan 번호에 의존하게 되고 (b) plan 디렉터리가 정리되면 dead reference 가 된다. 의도 (WHY) 는 PR description / commit message 에 기록.
**Good**: plan 번호만 제거, WHY 설명은 그대로 유지.
```ts
// KaTeX output:"html" 모드의 fallback 영역 aria-hidden 보존
describe("KaTeX math rendering", () => { ... });
```
**검출**: PR push 전 (또는 critic 의 코드 대조 단계):
```bash
git diff origin/main -- 'src/**' '*.css' | grep -nE "^\+.*\b(plan[0-9]+|task[- ]?[0-9]+)\b"
# 결과 있으면 의도 검토 후 제거. ADR 참조는 ADR-NNN 명시로 유지 가능
```

**Self-check**: 추가된 src / css / test 파일에서 plan 번호 / task 번호 grep 결과 0건? (PR description 만 plan 번호 허용)
...
- [ ] **1-10**: 코드 / 주석 / describe 에 plan 번호 / task 번호 grep 0건
...
- **BLG18. Drizzle MySQL `onDuplicateKeyUpdate` 의 `VALUES(literal)` deprecated 회피** (PR #146 관측). `set: { col: sql\`VALUES(col)\` }` 처럼 컬럼 이름을 literal 문자열로 박은 raw SQL 은 MySQL 8.0.20 부터 deprecated 경고 대상. 실행 자체는 되지만 로그에 warning 누적 + 향후 MySQL major 버전에서 제거 위험. **Good**: Drizzle column 참조 형태 `set: { col: sql\`VALUES(${categories.col})\` }` 또는 (Drizzle 0.45+) `set: { col: sql\`VALUES(${sql.identifier(...)})\` }` 사용. **검출**: `grep -rnE "sql\`VALUES\([a-z_]+\)\`" src/` 결과 — column 참조 없이 literal 만 있으면 위험. **Why**: Drizzle MySQL adapter 의 UPSERT 패턴은 자유도가 높아 literal 문자열도 그대로 통과 — type-check 단계에서 deprecated 가 잡히지 않으므로 review/문서에서 명시적 가드 필요.
- **BLG19. Drizzle `notInArray(col, [])` 빈 배열 동작이 버전 의존적** (PR #146 관측). 빈 배열을 넘기면 SQL 로 `WHERE col NOT IN ()` 가 생성되어 0 건 매칭 (MySQL 표준) — 즉 모든 row 가 통과해 의도와 정반대 효과 (`tx.delete().where(...)` → 0건 삭제). 그러나 Drizzle 의 사전 검증이 빈 배열을 SQL 빌드 단에서 단락시킬 수도 있어 버전마다 동작 차이. **Good**: 빈 배열 케이스 명시적 분기 — `if (currentNames.length === 0) { tx.delete(table); } else { tx.delete(table).where(notInArray(col, currentNames)); }`. 회귀 테스트에 빈 배열 케이스 1건 필수 (예: `expect(repo.syncAll).toHaveBeenCalledWith([])` + 모든 row 삭제 검증). **검출**: `grep -rnE "notInArray\([^,]+,\s*[a-zA-Z_$][a-zA-Z0-9_$]*\)" src/` 결과 중 분기 가드 없는 호출. **Why**: Drizzle 의 SQL builder 는 SQL 표준 동작과 라이브러리 단락 처리 사이에서 의도가 명확하지 않을 수 있음 — 명시적 분기가 의도 보존 + 버전 안전.
- **BLG20. git 공유 config 에 사용자별 절대경로 금지** (PR #151 관측). `.claude/settings.json` hook command 에 `cd /Users/nhn/personal/fos-blog` 처럼 로컬 머신 절대경로를 넣으면 다른 환경 / CI / 다른 개발자 머신에서 hook 즉시 실패. PreToolUse 와 PostToolUse 가 비대칭이면 더 잘 놓침. **Good**: `cd "$CLAUDE_PROJECT_DIR"` 또는 hook 스크립트 자체를 `$CLAUDE_PROJECT_DIR/.claude/hooks/*.sh` 로 참조. inline command 안의 cd 도 동일 placeholder. **검출**: `grep -nE '/Users/|/home/' .claude/settings.json .claude/hooks/*.sh`. **Why**: settings.json 은 git 추적 대상이라 머지 즉시 다른 환경 break. hook 실패는 silent 인 경우가 많아 한참 후에 발견.
- **BLG21. bash `eval` + unquoted 변수 전개 금지** (PR #151 관측). `FLAG="-C /path"` 후 `eval git $FLAG diff` 패턴은 경로에 공백 포함 시 word splitting + `eval` 자체로 임의 명령 실행 가능성 (외부 입력이 한 단계 거쳐 들어오면 RCE). hook 스크립트는 `$CMD` 같은 외부 입력을 자주 다루므로 특히 위험. **Good**: bash 배열 사용 — `flags=(-C "${BASH_REMATCH[1]}")` 후 `git "${flags[@]}" diff`. 빈 배열도 안전 (`git "${flags[@]}"` = `git` 호출). **검출**: `grep -rnE 'eval[[:space:]]+(git|sh|bash|[a-z]+)[[:space:]]+\$' .claude/hooks/`. **Why**: 배열 전개는 단어 경계가 보존되고 eval 의 재해석 단계가 없어 두 위험 모두 0.
- **BLG23. 보조 섹션의 DB 쿼리는 try-catch + 빈 폴백** (PR #152 관측). 글 상세 페이지의 "관련 글 / 인기 글 / 사이드바 카테고리" 같은 **본문이 아닌 보조 섹션** repository 메서드는 try-catch 없이 throw 되면 페이지 전체 렌더가 실패한다. 본문 (글 내용) 이 있는데 보조 섹션 DB 일시 장애로 페이지가 통째로 5xx 되는 건 UX 손실. **Good**: 보조 섹션 메서드 전체를 try-catch 로 감싸 `log.error({ err }, "...")` + 빈 배열/null 반환. 컴포넌트는 빈 배열일 때 `null` 반환해 섹션만 사라지게 (graceful degradation). 예: `getRelatedPosts` 의 catch → `[]`, `<RelatedPosts>` 의 `posts.length === 0 → null`. **검출**: `grep -nE "async (getRelated|getPopular|getSidebar|getRecent)" src/infra/db/repositories/*.ts` 결과 중 함수 본문에 `try` 없는 메서드. **Why**: 본문 (필수) 과 보조 (부가) 의 실패 영향을 분리. 본문 메서드 (`getPost`) 는 throw 가 옳음 (notFound / 5xx) — 보조만 graceful.
- **BLG24. `src/components/markdown/` 신규 모듈은 `import "server-only"` 필수** (PR #155 관측). 디렉터리 규칙상 모든 모듈이 server-only 가드를 가져야 client 번들에 우발적으로 포함되지 않는다. 기존 모듈을 import 하는 신규 파일이라도 **전이적 보호에 의존 금지** — 다른 import 경로가 생기면 그 시점부터 노출. **Good**: 신규 파일 최상단에 `import "server-only";` (다른 import 위, 빈 줄 분리). **검출**: `for f in src/components/markdown/*.ts; do head -3 "$f" | grep -q '"server-only"' || echo "$f: missing"; done`. **Why**: 디렉터리 단위 가드 규칙이 명시되어 있어도 신규 파일 작성 시 누락 가능 — 파일이 늘어날수록 누락 1건이 늘어날 확률 증가. `code-architecture.md:252` 의 규칙과 paired.
- **BLG22. SKILL.md 의 agent 타입 참조와 `.claude/agents/` 실제 파일 불일치 금지** (PR #151 관측). SKILL.md 본문에 "`oh-my-claudecode:writer` 를 spawn" 이라 적었지만 실제 `.claude/agents/` 에는 `self-healing-postmortem.md` 커스텀 에이전트 정의가 있고 호출 메커니즘이 다름. 더 위험한 점: agent-spawn-guard.sh 의 guard 목록에 OMC writer 가 없어 `team_name` 없이 spawn 해도 차단되지 않음 → 잘못된 타입이 silent 로 통과. **Good**: SKILL.md 작성 시 spawn 대상은 (a) `.claude/agents/<name>.md` 파일이 실제 존재하는지, (b) guard 목록과 정합한지 두 체크 후 확정. **검출**: `grep -nE 'spawn|subagent_type' .claude/skills/*/SKILL.md` 결과의 agent 이름이 `ls .claude/agents/` + agent-spawn-guard.sh 의 case 목록에 있는지 cross-check. **Why**: SKILL.md 는 team-lead 가 매 사이클 참조하는 운영 문서 — 잘못된 타입은 곧바로 실행 사고. 산출 파일 구조 섹션과 본문이 어긋나면 review 가 잡지 못하면 그대로 운영 진입.
