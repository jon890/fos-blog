...
| 섹션 | 카테고리 | 호출 시점 | 사용 스킬 |
|---|---|---|---|
| 섹션 1 | plan 작성 (critic 회피) | task 파일 작성 직후 self-check | `planning`, `build-with-teams` |
| 섹션 2 | team 운영 | 팀원 스폰 / 메시지 / 브랜치 작업 시 | `build-with-teams` |
| 섹션 3 | PR review 학습 (코드 패턴 함정) | 리뷰 댓글 처리 후 누적 | `review-fix` |
| 섹션 4 | 레포별 +α | task 도메인 코드 작성 시 | `planning`, `build-with-teams` |
...
| 1-1 | 수치 추측 (파일 수 / 줄 수) | 수치, git diff --stat, 실측 | plan 작성 |
| 1-2 | 파일 범위 부정확 | 파일 목록, --name-only, 전체 | plan 작성 |
| 1-3 | 이전 plan / main 커밋과의 상호작용 누락 | 커밋 상호작용, rebase, 충돌 | plan 작성 |
| 1-4 | 실행 컨텍스트 모호 (cwd / branch) | cwd, worktree, Bash 블록 | plan 작성 |
| 1-5 | "눈으로 확인" 검증 | 눈으로 확인, 기대값, grep | plan 작성 |
| 1-6 | 외부 상태 gate 부재 | gate, gh pr view, rollback | plan 작성 |
| 1-7 | 새 불변식 도입 시 4면 가드 누락 | 불변식, 4면 가드, Migration | plan 작성 |
| 1-8 | 마지막 phase 에 index.json `completed` 마킹 지시 누락 | index.json, completed, 마킹 | plan 작성 |
| 1-9 | macOS BSD `sed` `\b` 미지원 | sed, \b, perl | plan 작성 |
| **섹션 2. team 운영** | | | team-lead |
| 2-1 | 팀원 SendMessage 회신 누락 | SendMessage, 회신, inbox | team 운영 |
| 2-2 | 팀원 자발적 실행 | 자발 실행, idle, 시작 지시 | team 운영 |
| 2-3 | self-shutdown 패턴 (fos-blog 관측) | self-shutdown, spawn, idle | team 운영 |
| 2-4 | executor cwd 격리 (main repo 오염 방지) | cwd, worktree, main repo 오염 | team 운영 |
| 2-5 | executor scope 확장 자체 판단 | scope 확장, eslint-disable, @ts-ignore | team 운영 |
| 2-6 | critic v2 재평가 시 신 파일 미재읽기 | v2 재평가, Read tool, 신 파일 | team 운영 |
| 2-7 | code-reviewer 에 plan 비자명 설계 결정 미전달 | code-reviewer, 설계 결정, false positive | team 운영 |
| 2-8 | task 재분할 시 index.json 갱신 누락 | index.json, total_phases, phases 배열 | team 운영 |
| 2-9 | cwd 추적 + 양쪽 git status 검증 | pwd, git status, worktree | team 운영 |
| 2-10 | 브랜치 확인 누락 commit 사고 | git branch, commit, PR 브랜치 | team 운영 |
| **섹션 3. PR review 학습 (코드 패턴)** | | | code-reviewer, executor |
| 3-1 | Drizzle `count(*)` 의 `sql<T>` 타입 부정확 (PR #81) | count(*), sql<number>, BigInt | code review |
| 3-2 | SVG `<stop>` 의 `stopColor="var(--...)"` 미해결 (plan013) | SVG, stopColor, var() | code review |
| 3-3 | CSS custom property 키는 `as CSSProperties` 단언 필요 (PR #81) | --my-var, CSSProperties, TS2353 | code review |
| 3-4 | inline `style={{ color: ... }}` vs Tailwind arbitrary class (PR #81) | inline style, arbitrary class, text-[var(--)] | code review |
| 3-5 | 라이트 모드 selector — mockup 잔재 금지 (plan013) | .ab.light, :root:not(.dark), selector | code review |
| 3-6 | silent 테스트 회귀 (plan007-2) | 우회 정책, 테스트 입력값, RFC1918 | code review |
| 3-7 | `passNode: true` 누락 — hast-util-to-jsx-runtime default false (PR #83) | passNode, toJsxRuntime, hast node | code review |
| 3-8 | server-only 가드 + vitest node 환경 충돌 (PR #83) | server-only, vi.mock, vitest | code review |
| 3-9 | non-null assertion (`!`) 대신 타입 가드 (PR #83) | !, non-null, 타입 가드 | code review |
| 3-10 | derived counter 를 매 iter 재계산 (O(n²)) (PR #103) | slice, filter, 누산 변수 | code review |
| 3-11 | client component catch 의 console.error 만 두면 UX 침묵 (PR #105) | console.error, errorState, UX 침묵 | code review |
| 3-12 | useRef array 가 source 변경 시 stale 잔존 (PR #105) | useRef, stale, trim effect | code review |
| 3-13 | npm dep 제거 전 globals.css / config 의 직접 참조 grep 누락 (PR #107) | dep 제거, globals.css, pretendard | code review |
| 3-14 | hex 입력 string 을 parseInt 로 분해할 때 형식 검증 누락 (PR #107) | parseInt(hex, 16), NaN, HEX_PATTERN | code review |
| 3-15 | react-hook-form + zod 의 create/edit 모드 schema 미분리 (PR #108) | zodResolver, editSchema, schema 분리 | code review |
| 3-16 | 서버 raw error 메시지를 클라이언트 toast 로 직접 노출 (PR #108) | toast.error, error.message, 화이트리스트 | code review |
| 3-17 | Drizzle timestamp 컬럼이 API 응답 후 ISO string 직렬화 (PR #108) | Date, ISO string, 직렬화 | code review |
| 3-18 | task spec 의 외부 라이브러리 import 경로 인용이 실제 코드와 mismatch (plan044) | import 경로, hast-util-sanitize, rehype-sanitize | code review |
| 3-19 | sanitize schema 의 `["className", /^regex/]` 튜플이 기존 단순 `"className"` 과 OR 동작이라 무용 (plan044) | OR 동작, className 튜플, rehype-sanitize | code review |
| 3-20 | 모달 `fixed inset-0` 컨테이너 + 내부 `absolute` 버튼 클릭 가로채기 (plan039) | fixed inset-0, z-index, pointer-events-none | code review |
| **섹션 4. 레포별 +α 패턴 (BLG)** | | | code-reviewer, executor |
| BLG1 | Drizzle `db:push` 금지 (프로덕션) | db:push, db:generate, db:migrate | code review |
| BLG2 | pino 구조화 로그 컨텍스트 누락 | pino, 구조화 로그, 4-field | code review |
| BLG3 | GitHub sync silent 실패 금지 | Octokit, return null, SyncResult | code review |
| BLG4 | Markdown 렌더 XSS 회피 | rehype-raw, rehype-sanitize, XSS | code review |
| BLG5 | Next.js 16 proxy 규약 (NJS15 잔재 방지) | proxy.ts, middleware.ts, runtime config | code review |
| BLG6 | `"use client"` 잘못 마킹 사고 (plan014 관측) | use client, runSync, RSC | code review |
...
| BLG14 | phase 문서 테스트 스니펫의 자기완결성 (PR #119 관측) | import 누락, 빈 stub, allowlist | code review |
| BLG15 | Drizzle `.select()` 부분 필드 + 광범위 반환 타입 mismatch 금지 (PR #121 관측) | select, Pick, widening | code review |
| BLG16 | phase verification 스크립트의 단순 indexOf false positive 금지 (PR #122 관측) | indexOf, false positive, node -e | code review |
...
| BLG25 | `useEffect` 한 번 캡처 + 변경 dep 혼합 금지 (PR #161 관측) | useEffect, activeElement, dep [] | code review |
...
새 패턴을 추가하기 전 아래 4조건을 모두 통과해야 한다.
하나라도 미달이면 PR 답글·커밋 메시지로 끝내고 이 파일에 적재하지 않는다.

- **재발성** — 2회 이상 재발했거나 다른 코드에서도 날 구조적 가능성이 있다. 단발 인시던트는 미등록.
- **심각도** — 데이터 손상·빌드 전체 실패·보안 등 영향이 크다. 경미한 스타일 지적은 미등록.
- **도구로 못 잡음** — 린터·타입체커·테스트가 이미 잡는 건 추가하지 않는다(도구가 단일 소스).
- **추상화 가능** — 특정 인시던트를 넘어 일반화된다. 개별 사례 기록은 git log 에 위임.

**prune·automate 패스** — 회고가 ADD 로만 기울지 않게 주기적으로 정리한다(회고 10회마다 또는 분기 1회).

- **prune** — 가리키는 코드가 사라진 stale 패턴을 삭제하고, 같은 커널의 중복 패턴을 병합한다.
- **automate** — 린터 규칙·ast-grep·테스트로 승격 가능한 패턴은 도구로 옮기고 항목을 삭제한다.

**신규 패턴 작성 형식** — 이 파일이 이미 쓰는 두 형식만 사용한다(새 형식 발명 금지).

- **BLG# (블로그 고유 함정)** — 표 행: `| BLG# | 증상 요약 | 키워드(grep 대상) | 담당(code review 등) |`.
- **P# (일반 plan 함정)** — `## N-M. 제목` 섹션 + Bad / Good(해결+검출 명령) / Why. 검출 명령(grep 등)을 함께 적어 다음 작업의 사전 self-check 로 쓰이게 한다.

---

# 섹션 1. plan 작성 (critic 회피)

`/planning` 또는 `build-with-teams` 가 task 파일 작성 시 self-check. 이 섹션의 모든 항목을 plan 생성 **전에 사전 해소**하면 critic 이 1-shot APPROVE 할 확률이 높다.
...
plan 제출 전 10개 패턴 모두 self-check (자동 검출 5 / 사람 판단 5 — 자세한 grep 은 `planning/SKILL.md` "task 생성 직후 자동 검증" 섹션):
...
- `review-fix` 6.5단계에서 추출. 같은 PR 에서 ✅ 누적 / ❌ 누적 금지 분류 후 섹션 3 추가
...
## BLG1. Drizzle `db:push` 금지 (프로덕션)

**증상**: `pnpm db:push` 를 프로덕션에 사용.

**Good**: `pnpm db:generate` → SQL 커밋 → `pnpm db:migrate`. 로컬 한정 예외 + 커밋 전 revert.

## BLG2. pino 구조화 로그 컨텍스트 누락

**증상**: 로그에 필수 컨텍스트 필드 누락.

**Good**: 최소 `{component, operation, ...domainContext, err}` 4-field 포함.

## BLG3. GitHub sync silent 실패 금지

**증상**: Octokit 호출에서 `try { ... } catch { return null }` 패턴 → 실패 silent 무시.

**Good**: 구조화 로그 + throw 또는 `SyncResult.failure`.

## BLG4. Markdown 렌더 XSS 회피

**증상**: `react-markdown` + `rehype-raw` 조합에 `rehype-sanitize` 누락.

**Good**: `rehype-sanitize` 동반 또는 source 자기 소유 명시 주석.

## BLG5. Next.js 16 proxy 규약 (NJS15 잔재 방지)

**Good**: `src/proxy.ts` = NJS16 정식 file convention.
Root `middleware.ts` 금지.
`runtime` config 금지.
`middleware-manifest.json` 비었다고 dead code 판단 금지.

## BLG6. `"use client"` 잘못 마킹 사고 (plan014 관측)

**증상**: `useState` / `useEffect` / `onClick` / `navigator` / `window` / `document` 사용 0건인데 `"use client"` 가 붙어 있으면 RSC server 첫 패스에서 client 컴포넌트 트리 평가가 일어나 sync 처리 (예: `react-markdown` 의 `runSync`) 가 server 에서 실행됨.
shiki / mermaid 등 async 의존성이 있는 라이브러리와 결합하면 `runSync finished async` 같은 사고 발생.

**검출**: `grep -l '"use client"' src/components/*.tsx | xargs -I{} sh -c 'grep -L "useState\|useEffect\|useRef\|onClick\|onChange\|navigator\\.\|document\\.\|window\\." {} && echo "{}: 잘못된 use client 의심"'`

**Good**: 인터랙션이 정말 없으면 `"use client"` 제거 (server component).
일부만 인터랙션이면 island 분리 (CodeCard / Mermaid 패턴).

## BLG7. task 파일 테스트 스니펫에 `as any` 금지 (PR #92 관측)

**증상**: phase-XX.md 의 ts 코드 블록에 `(tree as any)` / `(c: any)` / `as string[]` 같은 escape hatch → type-check 가 unchecked 로 통과해 진짜 검증 의미 약화.

**Good**: hast 트리는 `import type { Root, Element, ElementContent } from "hast"` + `function isElement(n: ElementContent): n is Element` type guard 패턴.
union 가능 속성 (`Properties.className: string[] | string | number`) 은 `Array.isArray()` 가드.

**Why**: planning doc 수준에서 깨끗한 코드 예시 = executor 의 1-shot 통과율 ↑.

## BLG8. prose-scoped 클래스 셀렉터 `.prose` prefix 일관성 유지 (PR #93 관측)

**증상**: `.code-card-body` 처럼 마크다운 렌더러 안에서만 쓰이는 클래스의 자식 셀렉터를 `.prose` 없이 작성 → 미래에 다른 컨텍스트에서 사용 시 의도치 않은 스타일 누출.
plan017 의 shiki dual theme 규칙이 이 패턴을 어겨 리뷰 지적.

**Good**: `html.dark .prose .code-card-body pre span { ... }` — 기존 `.prose .code-card-body { padding: ... }` 규칙들과 일관.

**검출**: `grep -nE '\.code-card-body' src/app/globals.css` 결과 중 `.prose` prefix 없는 셀렉터.

**Why**: 일관된 스코프는 향후 컴포넌트 재사용 시 회귀 차단 + critic 이 globals.css 변경 평가 시 사전 점검할 패턴.

## BLG9. JSDoc/TSDoc 코멘트에 Tailwind 클래스 패턴 금지 (PR #94 관측)

**증상**: Tailwind v4 의 content scanner 가 `.ts`/`.tsx` 파일의 **코멘트 안 문자열까지** 클래스 후보로 추출 — `text-[var(--color-cat-*)]` 의 `*`, `text-[var(--color-cat-{key})]` 의 `{` `}` 가 그대로 layer utilities 에 invalid CSS 생성 → `Unexpected token Delim('*')` 또는 `CurlyBracketBlock` parse error → 모든 페이지 500.

**검출**: 코멘트에 `[var(--`, `text-[`, `bg-[` 같은 패턴이 있고 그 안에 `*` / `{` / `}` 가 들어 있으면 위험.

**Good**: 코멘트에서 클래스 패턴 자체를 prose 로 표현 (예: "color-cat 토큰 (canonical key 별)").
코드 예시 필요하면 `text-[var(--color-cat-X)] 형태` 로 placeholder 만 쓰기.
추가 안전망: `globals.css` 에 `@source not "../../tasks/**"` / `@source not "../../docs/**"` 추가 (tasks/ 와 docs/ 의 markdown 도 default scan 대상).
...
**증상**: full repository 인터페이스로 캐스팅하면 future 메서드 추가 시 mock 누락이 컴파일 단계에서 안 잡힘 — silent regression 위험.

**Good**: service 측에서 사용 메서드만 가진 좁은 인터페이스 (예: `StatsPostRepository`) 분리하고 `interface ServiceRepos { post: StatsPostRepository }` 로 받음.
test 의 `makeRepo()` 가 자연스레 인터페이스 만족, 캐스트 불필요.

**Why**: structural typing 활용 + future-proof. PostRepository 에 메서드 추가 시 service / test 모두 영향 없음.

## BLG12. co-located CSS 페이지의 inline style 금지 (PR #114 관측)

**증상**: `src/app/<page>/<page>.css` 가 이미 있는 페이지에서 `style={{ fontFamily: "var(...)", fontSize: 11, ... }}` inline style → 일관성 깨짐 + 같은 토큰 다른 표기.

**Good**: CSS 파일에 `.<page>-<region> .<modifier>` 클래스 추가 후 `<span className="modifier">`.

**검출**: `grep -nE 'style=\{\{' src/app/**/page.tsx` 결과 중 `<page>.css` 가 동일 디렉터리에 있는 파일.

**Why**: 단일 소스 (디자인 토큰 + 레이아웃 모두 CSS 파일).

## BLG13. TS narrowing 후 `as` 단언 금지 (PR #118 관측)

**증상**: `if (frontMatter.description) { return frontMatter.description as string; }` 처럼 narrowing 으로 이미 `string` 추론된 자리에 `as string` 을 붙이면, 향후 타입 정의 변경 시 실제 타입 에러를 단언이 가려 silent 회귀.

**Good**: 단언 제거 → `frontMatter.description.replace(...)` 직접 호출.
narrowing 이 안 되면 타입 가드 함수 분리.

**검출**: `grep -nE '\) as (string\|number\|boolean)\b' src/**/*.ts` 결과 중 narrowing 직후 라인.

**Why**: TS strict 의 가치는 변경 시 컴파일러가 실제 위험을 알려주는 것 — 단언은 그 신호를 차단.

## BLG14. phase 문서 테스트 스니펫의 자기완결성 (PR #119 관측)

**증상**: phase-XX.md 의 테스트 코드 블록은 executor 가 거의 그대로 복사 → 누락 / 빈 stub / 검증 부족이 그대로 프로덕션 테스트가 됨.
3가지 흔한 결함:
- import 구문 누락 (`toHtml(tree)` 사용하면서 `import { toHtml } from "hast-util-to-html"` 빠짐 → `ReferenceError`)
- 빈 stub (`it("...", async () => { /* TODO */ })` 통과하지만 실제 검증 없음 — 보안/회귀 검증 무의미)
- allowlist 정합성 부족 (sanitize/schema 변경 시 일부 속성만 검증, 다른 속성 누락 silent 회귀)

**Good**: phase 의 모든 테스트 스니펫에:
- (a) 상단 import 블록 명시 (`vitest`, 외부 lib, project module 모두)
- (b) 모든 `it(...)` 에 최소 1개 `expect`
- (c) schema/allowlist 가 등록한 속성마다 보존 케이스

**검출**: `grep -nE 'it\([^)]*async \(\) => \{ /\*' tasks/**/phase-*.md` (빈 stub 의심)

**Why**: planning doc 의 코드 = executor 의 1-shot 통과율 + critic 의 사후 review 부담을 결정.

## BLG15. Drizzle `.select()` 부분 필드 + 광범위 반환 타입 mismatch 금지 (PR #121 관측)

**증상**: `db.select({ title, path, slug, category, ... })` 처럼 일부 컬럼만 선택하면서 반환 타입을 `Promise<PostData[]>` 같은 전체 타입으로 명시 → 호출자가 누락 필드 (예: `updatedAt`) 에 접근해도 컴파일러가 막지 않음 → 런타임 `undefined` silent 회귀.

**Good**: 반환 타입을 `Pick<PostData, "title" | "path" | "slug" | ...>[]` 로 좁히거나 전용 alias (`RSSPostData`) 분리.
service 레이어에서 `export type RSSPostData = Pick<PostData, ...>` 로 재사용.

**검출**: `.select({ ... })` 와 함수 시그니처의 `Promise<XData[]>` 가 키 집합 불일치.
코드 리뷰 시 select 키 vs 반환 타입 키 매핑 점검.

**Why**: Drizzle 의 type inference 는 select 객체에서 정확한 타입을 추론하지만, 명시적 반환 타입이 그 추론을 widening 으로 덮어씀.
이후 호출자가 `r.updatedAt` 처럼 누락 필드에 접근해도 strict 모드라도 통과.

## BLG16. phase verification 스크립트의 단순 indexOf false positive 금지 (PR #122 관측)

**증상**: `node -e "...indexOf('CategoryList')..."` 처럼 단순 문자열 검색으로 JSX 위치를 비교 → import 라인 / 주석 / 다른 등장을 잡아 false positive 발생.
plan030 phase-01 의 검증 스크립트가 import 라인을 잡아 ORDER 가 거꾸로 판정된 실례.

**Good**: JSX 태그 패턴 (`'<CategoryList categories'`) 또는 조건문 패턴 (`'popularPosts.length > 0 &&'`) 처럼 위치 고유한 substring 사용.
또는 grep 의 `-n` 으로 라인 번호를 받아 비교.

**검출**: phase-XX.md 의 `node -e` 블록 중 `indexOf('<단어>')` 형태가 있고 해당 단어가 import/타입 선언 등에서도 등장 가능하면 위험.

**Why**: planning 시점의 검증 스크립트는 그대로 executor 의 success criteria 가 되므로 false positive/negative 양쪽 모두 task 통과율을 흐림.
위치 고유 substring 으로 명시해야 한다.

## BLG17. connection string / 시크릿이 에러 메시지를 통해 로그에 노출되는 패턴 금지 (PR #124 관측)

**증상**: MySQL 드라이버 (`mysql2`) / Redis / Octokit 등 외부 클라이언트는 connection 실패 시 `error.message` 에 host/port/username/password 가 포함된 raw URL 일부를 그대로 넣는 경우가 많다.
`console.log/error(error.message)` 또는 `logger.error({ err })` 로 출력하면 컨테이너 로그 / pino 구조화 로그에 비밀번호가 잔류 → 운영 로그 유출 위험.

**Good**: 출력 직전에 마스킹 helper 적용 (`message.replace(/:([^@/]*)@/, ":***@")`).
필요 시 `lib/` 의 공용 `maskSecret()` 유틸로 분리.

**검출**: `grep -rnE 'console\.(log|error)\([^)]*error.*message|err\.message' src/ scripts/` 결과 중 connection string 을 다루는 라이브러리 호출 (mysql.createConnection / redis.createClient / Octokit init / fetch with auth header 등) 의 catch 블록.

**Why**: connection 실패는 정상 운영에서도 자주 발생 (네트워크 일시 불안정 / DB 재시작) 하므로 마스킹 누락은 실시간으로 노출이 누적된다.
`restart=unless-stopped` 와 결합되면 더 빈번.

## BLG18. Drizzle MySQL `onDuplicateKeyUpdate` 의 `VALUES(literal)` deprecated 회피 (PR #146 관측)

**증상**: `set: { col: sql\`VALUES(col)\` }` 처럼 컬럼 이름을 literal 문자열로 박은 raw SQL → MySQL 8.0.20 부터 deprecated 경고 대상.
실행 자체는 되지만 로그에 warning 누적 + 향후 MySQL major 버전에서 제거 위험.

**Good**: Drizzle column 참조 형태 `set: { col: sql\`VALUES(${categories.col})\` }` 또는 (Drizzle 0.45+) `set: { col: sql\`VALUES(${sql.identifier(...)})\` }` 사용.

**검출**: `grep -rnE "sql\`VALUES\([a-z_]+\)\`" src/` 결과 — column 참조 없이 literal 만 있으면 위험.

**Why**: Drizzle MySQL adapter 의 UPSERT 패턴은 자유도가 높아 literal 문자열도 그대로 통과 — type-check 단계에서 deprecated 가 잡히지 않으므로 review/문서에서 명시적 가드 필요.

## BLG19. Drizzle `notInArray(col, [])` 빈 배열 동작이 버전 의존적 (PR #146 관측)

**증상**: 빈 배열을 넘기면 SQL 로 `WHERE col NOT IN ()` 가 생성되어 0 건 매칭 (MySQL 표준) — 즉 모든 row 가 통과해 의도와 정반대 효과 (`tx.delete().where(...)` → 0건 삭제).
Drizzle 의 사전 검증이 빈 배열을 SQL 빌드 단에서 단락시킬 수도 있어 버전마다 동작 차이.

**Good**: 빈 배열 케이스 명시적 분기 — `if (currentNames.length === 0) { tx.delete(table); } else { tx.delete(table).where(notInArray(col, currentNames)); }`.
회귀 테스트에 빈 배열 케이스 1건 필수 (예: `expect(repo.syncAll).toHaveBeenCalledWith([])` + 모든 row 삭제 검증).
...
## BLG23. 보조 섹션의 DB 쿼리는 try-catch + 빈 폴백 (PR #152 관측)

**증상**: 글 상세 페이지의 "관련 글 / 인기 글 / 사이드바 카테고리" 같은 보조 섹션 repository 메서드는 try-catch 없이 throw 되면 페이지 전체 렌더가 실패.
본문 (글 내용) 이 있는데 보조 섹션 DB 일시 장애로 페이지가 통째로 5xx 되는 건 UX 손실.

**Good**: 보조 섹션 메서드 전체를 try-catch 로 감싸 `log.error({ err }, "...")` + 빈 배열/null 반환.
컴포넌트는 빈 배열일 때 `null` 반환해 섹션만 사라지게 (graceful degradation).
예: `getRelatedPosts` 의 catch → `[]`, `<RelatedPosts>` 의 `posts.length === 0 → null`.

**검출**: `grep -nE "async (getRelated|getPopular|getSidebar|getRecent)" src/infra/db/repositories/*.ts` 결과 중 함수 본문에 `try` 없는 메서드.

**Why**: 본문 (필수) 과 보조 (부가) 의 실패 영향을 분리.
본문 메서드 (`getPost`) 는 throw 가 옳음 (notFound / 5xx) — 보조만 graceful.

## BLG24. `src/components/markdown/` 신규 모듈은 `import "server-only"` 필수 (PR #155 관측)

**증상**: 디렉터리 규칙상 모든 모듈이 server-only 가드를 가져야 client 번들에 우발적으로 포함되지 않는다.
기존 모듈을 import 하는 신규 파일이라도 **전이적 보호에 의존 금지** — 다른 import 경로가 생기면 그 시점부터 노출.

**Good**: 신규 파일 최상단에 `import "server-only";` (다른 import 위, 빈 줄 분리).
...
**증상**: 단일 useEffect 에 `[index, ...]` 같은 변경 dep + 내부에 `const x = document.activeElement` 같은 "open 시점 1회 캡처" 가 공존 → dep 변경마다 cleanup+setup 전체 재실행 → 캡처가 매번 새로 됨.
lightbox focus return 의 경우 image 전환할 때마다 닫기 버튼 자체가 캡처되어 trigger 복귀 실패.

**Good**: 효과를 두 개로 분리 —
- (a) mount/unmount 전용 effect (dep `[]`) + `useRef` 로 캡처값 보관
- (b) 변경 dep effect 는 keyboard handler 등 매 재실행 안전한 로직만

**검출**: `grep -rnE 'useEffect\(' src/components/` 결과 중 dep 배열에 변경 가능 prop/state 가 있으면서 본문에 `document\.activeElement|getBoundingClientRect|window\.(scroll|inner|outer)` 같은 1회 측정 캡처가 함께 있는 케이스.

**Why**: useEffect 는 dep 변경마다 cleanup → setup 재실행이 React 의 동작 정의.
"한 번만" 의도는 dep `[]` 가 유일한 표현이고, 1회 캡처값을 다른 dep effect 에서 참조하려면 `useRef` 가 표준.

---

## BLG26. Tailwind `last:` 변형은 직접 부모의 last-child 기준 — wrapper 도입 시 시맨틱 변화 주의 (PR #167 관측)

**증상**: 형제 카드 목록의 "마지막 카드에만 border-b" 의도로 `last:border-b` 를 카드 element 에 부여.
이후 다른 목적 (container query / animation wrapper / portal 등) 으로 wrapper div 를 한 겹 도입하면 `last:border-b` 가 wrapper 의 유일한 자식이 되어 **항상 last-child = true** 가 됨 → 모든 카드에 border 적용 (또는 그 반대로 wrapper 가 부모 형제 중 last 가 아니라 어디서도 적용 안 됨).

**Good**: wrapper 를 도입할 때 `last:` / `first:` 변형은 wrapper 쪽으로 함께 이동.
- Before: `<Card className="border-t last:border-b">`
- After (wrapper 도입): `<div className="@container last:border-b"><Card className="border-t">`

부모-자식 관계상 어느 element 가 "형제 last" 가 되는지 한 번 더 확인.

**검출**: `grep -nE 'last:(border|mt|pt|mb|pb|ml|pl|mr|pr)' src/components/` 결과 element 가 단일 자식 wrapper 안으로 들어간 후 그대로 남아 있는지 점검. PR review 단계에서 "wrapper 도입 + 기존 first:/last: 그대로 유지" 패턴 발견 시 항상 의심.

**Why**: CSS `:last-child` 는 *직접 부모의 마지막 자식* 만 매칭. Tailwind `last:` 도 같은 시맨틱. 시각 결과만 검증하면 "원래도 모든 카드에 border 있었다" 처럼 잘못 인지 가능 — 실측 (`getComputedStyle(el).borderBottomWidth`) 로 idx 별로 확인해야 정확.
