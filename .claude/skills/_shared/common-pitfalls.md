...
- **BLG25. `useEffect` 한 번 캡처 + 변경 dep 혼합 금지** (PR #161 관측). 단일 useEffect 에 `[index, ...]` 같은 변경 dep + 내부에 `const x = document.activeElement` 같은 "open 시점 1회 캡처" 가 공존하면 dep 변경마다 cleanup+setup 전체 재실행 → 캡처가 매번 새로 됨. lightbox focus return 의 경우 image 전환할 때마다 닫기 버튼 자체가 캡처되어 trigger 복귀 실패. **Good**: 효과를 두 개로 분리 — (a) mount/unmount 전용 effect (dep `[]`) + `useRef` 로 캡처값 보관, (b) 변경 dep effect 는 keyboard handler 등 매 재실행 안전한 로직만. **검출**: `grep -rnE 'useEffect\(' src/components/` 결과 중 dep 배열에 변경 가능 prop/state 가 있으면서 본문에 `document\.activeElement|getBoundingClientRect|window\.(scroll|inner|outer)` 같은 1회 측정 캡처가 함께 있는 케이스. **Why**: useEffect 는 dep 변경마다 cleanup → setup 재실행이 React 의 동작 정의. "한 번만" 의도는 dep `[]` 가 유일한 표현이고, 1회 캡처값을 다른 dep effect 에서 참조하려면 `useRef` 가 표준.
