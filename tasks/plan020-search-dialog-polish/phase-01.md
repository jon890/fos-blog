...
| `bg-black/50 backdrop-blur-sm` (백드롭) | **그대로 유지** — `--color-bg-overlay` 는 elevated surface 배경 토큰(라이트 = #ededef)이라 scrim 으로 쓰면 라이트 모드에서 불투명 연회색 레이어가 되어 모달 dimmer 동작 깨짐. 표준 black/50 유지가 양 모드 모두 자연스러움 |
...
| `hover:bg-gray-100 dark:hover:bg-gray-800` (X 닫기 버튼, line 111) | `hover:bg-[var(--color-bg-subtle)]` |
...
| `rounded-xl shadow-2xl` (다이얼로그) | `rounded-[12px] shadow-[var(--shadow-modal)]` |

**확인 명령**:
```bash
# cwd: <repo root>
# 백드롭의 의도된 bg-black/50 만 1줄 남고 나머지 0줄이어야 함
grep -nE "bg-white|bg-gray-|text-gray-|border-gray-|dark:bg-gray|dark:text-gray|dark:border-gray|dark:hover:bg-gray" src/components/SearchDialog.tsx
# 결과 0줄
```
...
**주의**: ArrowDown/Up/Enter 핸들러는 `results.length` / `results[selectedIndex]` 를 참조하므로 stale closure 회피 필요. 기존 ESC `useEffect([isOpen, onClose])` 에 묶지 말고 **별도 `useEffect` 로 분리** 하거나 `results`, `selectedIndex` 를 dep array 에 추가.
...
| 키보드 핸들러 stale closure (`results.length` 가 fetch 이전 값) | ArrowDown/Up/Enter 핸들러를 ESC effect 와 분리된 별도 useEffect 로 두고 `[isOpen, results, selectedIndex]` 를 dep array 에 명시 |
