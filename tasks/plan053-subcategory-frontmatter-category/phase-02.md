...
pnpm test --run src/infra/db/repositories/PostRepository.test.ts
pnpm lint
pnpm type-check
pnpm build
...
`PostRepository.test.ts`가 존재하지 않거나 새로 만들기 어렵다면, 실제 추가한 repository 테스트 파일 경로로 두 번째 `pnpm test --run` 명령을 바꾼다.
