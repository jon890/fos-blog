...
**`.gitattributes` binary 등록 (필수)**: 신규 폰트 파일을 `*.ttf binary` / `*.woff binary` 형태로 `.gitattributes` 에 등록. 이미 패턴 등록되어 있으면 추가 불필요.
 ...
 *
 * 키는 정규화된 짧은 alias 기준 (ai/db/js/...). categoryIcons 의 풀네임 (database/javascript)
 * 은 ALIAS 매핑으로 흡수. categoryIcons 에 없는 키 (next/system 등) 는 forward-compat 잉여물 —
 * 추가 카테고리 도입 시 별도 plan 에서 구현.
 */
...
**카테고리 정합 (필수)**: executor 는 `src/infra/db/constants.ts` 의 `categoryIcons` 키 전체를 먼저 확인. `OG_CATEGORY_HEX` 에 직접 키가 있거나 `OG_CATEGORY_ALIAS` 로 흡수되거나 fallback 으로 가는지 정합 검증. 신규 카테고리 추가 시 alias 또는 직접 키 양쪽 중 하나만 추가하면 동작.
...
### 5. `src/lib/og.test.ts` 신규 케이스 추가

`OG_COLORS` 키 직접 검증 테스트는 현재 없음 (`truncateForOg` 단위 테스트만). 기존 테스트 그대로 유지하고 신규 케이스만 추가:
- `getCategoryHex("js")` → `OG_CATEGORY_HEX.js` 반환
- `getCategoryHex("javascript")` → `OG_CATEGORY_HEX.js` 반환 (alias)
- `getCategoryHex("database")` → `OG_CATEGORY_HEX.db` 반환 (alias)
- `getCategoryHex("UNKNOWN")` → `OG_CATEGORY_DEFAULT_HEX` 반환
- `hexWithAlpha("#3fbac9", 0.12)` → `"rgba(63, 186, 201, 0.12)"` 반환

## Completion Gate

작업 항목 1~5 완료 후 아래 모두 통과해야 phase 종료:
...
grep -n "OG_CATEGORY_ALIAS\|database.*db\|javascript.*js" src/lib/og.ts   # alias 정의 확인
grep -n "export.*hexWithAlpha" src/lib/og.ts                                # 헬퍼 export 확인
...
- database / javascript 카테고리 글에서 fallback teal 이 아닌 고유 hex 색이 적용되는지 확인 (alias 정합)

### task status 마킹

`tasks/plan021-og-image-refresh/index.json` 의 phase 1 + 최상위 `status` 를 `"completed"` 로 변경. 모든 코드 변경과 함께 단일 commit 으로 묶음.
...
| `src/lib/og.ts` | 수정 (OG_COLORS 교체 + OG_CATEGORY_HEX + OG_CATEGORY_ALIAS + getCategoryHex + hexWithAlpha + loadOgFont path) |
