...
### 4. `tasks/plan015-categories-redesign/index.json` 완료 마킹

```bash
# cwd: <worktree root>
# index.json 의 status 와 모든 phase status 를 "completed" 로 업데이트
# (executor 가 jq + Write 또는 직접 편집)
```

`index.json` 의 최상위 `status: "pending"` → `"completed"` 로, `phases[].status` 모두 `"pending"` → `"completed"` 로 일괄 변경. 이 변경은 PR 브랜치 안에서 commit (별도 commit 가능). main 직접 push 금지.

---

## Critical Files

이 phase 는 코드 변경 없음. 검증 + 잔재 정리 + 완료 마킹만:
