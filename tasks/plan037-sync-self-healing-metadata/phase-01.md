...
| Drizzle MySQL UPSERT API | `.onDuplicateKeyUpdate({ set: { col: sql\`VALUES(${categories.col})\` } })` | Drizzle MySQL adapter 표준 패턴. PostgreSQL 의 `onConflictDoUpdate` 와 다름. column 참조 형태로 MySQL 8.0.20+ deprecated `VALUES(literal)` 경고 회피 |
| icon 컬럼 타입 | `string \| null` | schema `varchar("icon", { length: 50 })` 가 nullable. 파라미터 타입 widening 으로 schema 일치 |
...
`icon: string | null` 은 schema 의 `varchar("icon", { length: 50 })` 가 nullable 이므로 타입 일치. UPSERT 의 `sql\`VALUES(${categories.icon})\`` 는 Drizzle column 참조로 MySQL 8.0.20+ deprecated `VALUES(icon)` 경고 회피.
...
**4-3. `MetadataSyncService.test.ts` — 빈 stats 케이스**

`syncAll([])` 호출 시 동작 검증 — `notInArray(col, [])` 가 Drizzle 버전마다 다르게 동작 가능 (`0건 삭제` vs `전체 삭제`) 라 명시적 분기 (`currentNames.length === 0` → `tx.delete(categories)`) 로 처리한 의도가 회귀로 깨지지 않는지 가드:

```ts
it("post 가 0 건이면 syncAll([]) 호출되어 모든 카테고리 row 삭제", async () => {
  postRepo.getCategoryStats.mockResolvedValue([]);
...
**4-4. `CategoryRepository` 단위 테스트 신규 (선택)**

테스트가 실제 DB 또는 in-memory MySQL 사용한다면 unit test 추가. 현재 repo 단위 테스트가 없으면 OOS — service 레이어 테스트에 위임.

### 5. 자동 verification

**중요 — atomic commit**: 작업 1 (CategoryRepository.syncAll 신규) + 작업 2 (MetadataSyncService 호출 갱신) + 작업 3 (SyncService short-circuit) 세 변경은 **단일 commit** 으로 묶는다. 분리 commit 시 중간 상태에서 `syncAll` 메서드 미존재로 컴파일 실패. 회귀 테스트 (작업 4) 는 같은 commit 또는 직후 commit 중 선택.
...
# atomic commit 가드 — 작업 1+2+3 이 한 commit 안에 같이 있는지
git log -1 --name-only HEAD | grep -cE "(CategoryRepository|MetadataSyncService|SyncService)\.ts"  # 3 (4 일 수도 — test 포함 시)
```
