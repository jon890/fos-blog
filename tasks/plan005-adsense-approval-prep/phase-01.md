...
- **실패 시 fallback** (BLG3 사일런트 실패 금지):
  - `logger.child({ module: "app/about" }).warn({ component: "about", operation: "github-profile", err: error instanceof Error ? error : new Error(String(error)), status: res?.status }, "github profile fetch failed")`
  - `err`/`status` 는 변수값(`res.status` 숫자) 을 실제로 전달 — 키만 쓰지 말 것
...
# 4) About 의 GitHub API 호출 + ISR 1시간 (page-level + fetch-level 모두)
grep -n "api.github.com/users/jon890" src/app/about/page.tsx
grep -nE "^export const revalidate = 3600" src/app/about/page.tsx
grep -nE "next:\s*\{[^}]*revalidate:\s*3600" src/app/about/page.tsx
