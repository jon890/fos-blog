...
규율 섹션 끝에 신규 패턴 작성 형식을 못박는다 — 이 파일이 이미 쓰는 **두 형식을 그대로 따른다**(새 형식 발명 금지).

- **BLG# (블로그 고유 함정)** — 표 행: `| BLG# | 증상 요약 | 키워드(grep 대상) | 담당(code review 등) |`.
- **P# (일반 plan 함정)** — `## N-M. 제목` 섹션 + Bad / Good(해결+검출 명령) / Why.

검출 명령(grep 등)을 함께 적어 다음 작업의 사전 self-check 로 쓰이게 한다.
...
# 기존 패턴 수 보존(내용 미변경) — BLG 는 표 행, P 는 ## N-M 섹션 헤더(두 형식이 실제 파일 형식)
grep -cE "^\| BLG[0-9]" .claude/skills/_shared/common-pitfalls.md    # BLG 표 행 개수 — 추가 전과 동일해야
grep -cE "^## [0-9]+-[0-9]+\." .claude/skills/_shared/common-pitfalls.md   # P 패턴(## N-M.) 개수 — 추가 전과 동일해야
```
